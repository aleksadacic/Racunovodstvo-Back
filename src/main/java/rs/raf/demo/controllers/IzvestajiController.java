...
public class IzvestajiController {
    ...
    public ResponseEntity<?> getIzvestaj(@RequestParam(defaultValue = ApiUtil.DEFAULT_PAGE) @Min(ApiUtil.MIN_PAGE) Integer page,
                                         @RequestParam(defaultValue = ApiUtil.DEFAULT_SIZE) @Min(ApiUtil.MIN_SIZE) @Max(ApiUtil.MAX_SIZE) Integer size,
                                         @RequestParam(defaultValue = "") String[] sort, Principal principal) throws DocumentException {

        byte[] pdf = kontnaGrupaService.makeBrutoBilansTableReport(principal.getName()).getReport();
